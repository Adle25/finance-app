...
export const ImportCard = ({ data, onCancel, onSubmit }: Props) => {
    const [selectedColumns, setSeletctedColumns] = useState<SelectedColumnsState>({});
    const headers = data[0];
    const body = data.slice(1);

    const onTableHeadSelectChange = (columnIndex: number, value: string | null) => {
        setSeletctedColumns((prev) => {
    ...
    const handleContinue = () => {
        ...
        const formatedData = arrayOfData.map((row) => ({
            ...row,
            amount: converAmountToMilliUnits(parseFloat(row.amount)),
            ...
            <Card className="border-none drop-shadow-sm">
                <CardHeader className="gap-y-2 lg:flex-row lg:items-center lg:justify-between">
                    ...
                    <div className="flex flex-col lg:flex-row gap-y-2 items-center gap-x-2">
                        ...
                        <Button className="w-full lg:w-auto" size="sm" disabled={progress < requiredOptions.length} onClick={handleContinue}>Continue ({progress} / {requiredOptions.length})</Button>
                    </div>
                </CardHeader>
                <CardContent>
                    <ImportTable
